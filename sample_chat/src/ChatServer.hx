...
class Client extends Connection{
  ...
  override public function onopen():Void {
    ...
    for(con in Client.clients) con.chatNotify(-1, "info", "ID:"+Std.string(myid)+"さんが来たよ〜");
  }

  override public function onclose():Void {
    clients.remove(myid);
    trace("onclose - ID:" + Std.string(myid));
    for(con in Client.clients) con.chatNotify(-1, "info", "ID:"+Std.string(myid)+"さんが帰ったよ〜");
  }
...
class ChatServer {
  ...
  public static function tick() {
    ...
    trace("tick" + fugaaaa);
    for(con in Client.clients) con.chatNotify(-2, "info", "現在"+fugaaaa+"人が接続中。");
  }
}
